...
# --- Page Config ---
st.set_page_config(page_title="Pro BEP Analyzer", layout="wide")
...
target_payback_months = st.sidebar.number_input("Target Payback (Months):", value=12, min_value=1)
...
# The profit needed every month to recover setup cost in time
required_monthly_profit = setup_cost / target_payback_months

# Mode specific inputs with safety checks to avoid the error in your screenshot
if mode == "Calculate BEP Units":
    ...
    if margin > 0:
        ...
        payback_units = (total_fixed_monthly + required_monthly_profit) / margin
        required_revenue = payback_units * adj_price
    else:
    ...
    # Logic for Price (Safe from division by zero)
    bep_adj_price = (total_fixed_monthly / units_sold) + var_costs
    bep_price = bep_adj_price / (1 - (discount / 100))
    
    payback_adj_price = ((total_fixed_monthly + required_monthly_profit) / units_sold) + var_costs
    payback_price = payback_adj_price / (1 - (discount / 100))
    required_revenue = units_sold * payback_adj_price
...
with st.container():
    ...
    if mode == "Calculate BEP Units":
        data = {
            "Metric": ["Break-Even Point", f"Target for {target_payback_months}-Mo Payback", "Required Monthly Revenue"],
            "Value": [f"{int(bep_units)} Units/Mo", f"{int(payback_units)} Units/Mo", f"${required_revenue:,.2f}"]
        }
    else:
        data = {
            "Metric": ["Break-Even Price", f"Price for {target_payback_months}-Mo Payback", "Required Monthly Revenue"],
            "Value": [f"${bep_price:,.2f}", f"${payback_price:,.2f}", f"${required_revenue:,.2f}"]
        }
...
# Graph always displays and transitions smoothly as inputs change
...
if mode == "Calculate BEP Units":
    ...
    ax.axvline(bep_units, color='white', linestyle='--', alpha=0.6, label='Break-Even')
    ax.axvline(payback_units, color='#f1c40f', linestyle=':', label='Payback Goal')
    ax.set_xlabel("Units Sold")
else:
    p_range = np.linspace(var_costs * 1.1, payback_price * 1.5, 100)
    profit = (units_sold * (p_range * (1 - (discount/100)) - var_costs)) - total_fixed_monthly
    
    ax.plot(p_range, profit, color='#3498db', lw=2.5, label='Monthly Profit')
    ax.axhline(0, color='white', lw=1, alpha=0.5)
    ax.axhline(required_monthly_profit, color='#f1c40f', linestyle=':', label='Required Profit for Goal')
    ax.axvline(payback_price, color='#2ecc71', linestyle='--', label='Target Price')
