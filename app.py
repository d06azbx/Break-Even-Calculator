...
# New Toggle for Timeframe
timeframe = st.sidebar.radio("View Results As:", ["Monthly", "Yearly"])
time_factor = 12 if timeframe == "Yearly" else 1

st.sidebar.markdown("---")
# Core inputs (No currency symbols)
...
# Adjusted for selected timeframe
total_fixed_adjusted = total_fixed_monthly * time_factor
...
if mode == "Calculate BEP Units":
    ...
    est_monthly_sales = st.sidebar.number_input(f"Estimated {timeframe} Sales:", value=100 * time_factor, min_value=1)
    
    adj_price = price * (1 - (discount / 100))
    margin = adj_price - var_costs
    
    if margin > 0:
        bep_units = total_fixed_adjusted / margin
        required_revenue = bep_units * adj_price
        # Payback is always in months to recover setup cost
        monthly_profit = ((est_monthly_sales / time_factor) * margin) - total_fixed_monthly
    ...
    units_sold = st.sidebar.number_input(f"{timeframe} Units Sold:", value=100 * time_factor, min_value=1)
    bep_adj_price = (total_fixed_adjusted / units_sold) + var_costs
...
st.subheader(f"Key Simulation Results ({timeframe})")

if mode == "Calculate BEP Units":
    payback_text = f"{payback_months:.1f} Months" if payback_months != float('inf') else "Never"
    data = {
        "Metric": [f"Break-Even Point ({timeframe})", f"Required {timeframe} Revenue", "Payback Period"],
        "Value": [f"{int(bep_units)} Units", f"{required_revenue:,.2f}", payback_text]
    }
else:
    data = {
        "Metric": [f"Break-Even Price ({timeframe})", f"Required {timeframe} Revenue"],
...
if mode == "Calculate BEP Units":
    ...
    rev = x * adj_price
    costs = total_fixed_adjusted + (x * var_costs)
    ...
    ax.set_xlabel(f"Units Sold ({timeframe})")
else:
    p_range = np.linspace(var_costs * 1.1, bep_price * 2, 100)
    profit = (units_sold * (p_range * (1 - (discount/100)) - var_costs)) - total_fixed_adjusted
    
    ax.plot(p_range, profit, color='#3498db', lw=2.5, label=f'{timeframe} Operating Profit')
